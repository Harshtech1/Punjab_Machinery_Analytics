// ==============================================================================
// Punjab Government Decision Support Dashboard - V7 (Executive)
...
// Districts for boundaries ONLY (No text rendering to prevent description length errors)
...
mainPanel.add(ui.Label({
  value: 'Punjab Village Decision Support',
...
var thematicLayersMap = {
  ...
  'Priority Index': 'Intervention_Priority'
};
...
buttonsPanel.add(ui.Label('━━━━━━━━━━━━━━━━━', {margin: '0 0 5px 0', color: '#cbd5e0'}));
buttonsPanel.add(ui.Label('THEMATIC LAYERS', {fontWeight: 'bold', fontSize: '14px', margin: '0 0 5px 0', color: '#2d3748'}));

var createBtn = function(name, layerKey) {
  return ui.Button({
    label: name,
    onClick: function() {
...
// Ensure buttons are created in the exact order specified
var orderedLayers = [
  'Methane', 'In-Situ', 'Ex-Situ', 'Prime Movers', 'General', 
  'CRM', 'SMAM', 'CDP', 'Policy Zones', 'Priority Index'
];

for (var i = 0; i < orderedLayers.length; i++) {
  var layerName = orderedLayers[i];
  buttonsPanel.add(createBtn(layerName, thematicLayersMap[layerName]));
}

buttonsPanel.add(ui.Label('━━━━━━━━━━━━━━━━━', {margin: '5px 0 0 0', color: '#cbd5e0'}));
mainPanel.add(buttonsPanel);

// ------------------------------------------------------------------------------
// 6. DISTRICT DROPDOWN NAVIGATION
// ------------------------------------------------------------------------------
...
function updateMapLayers() {
  ...
  if (key === 'Policy_Zon') {
    ...
    if (['In_Situ', 'Ex_Situ', 'Prime_Move', 'General', 'CRM', 'SMAM', 'CDP'].indexOf(key) > -1) {
...
function updateLegend() {
  ...
  var title = 'Legend';
  for (var k in thematicLayersMap) {
    if (thematicLayersMap[k] === key) {
      title = k;
      break;
    }
  }
  
  legendPanel.add(ui.Label(title, {fontWeight: 'bold', margin: '0 0 10px 0'}));
...
Map.onClick(function(coords) {
  ...
  clickedFeature.evaluate(function(feature) {
    ...
    for (var i = 0; i < machFields.length; i++) {
      var f = machFields[i];
