// ==============================================================================
// Punjab Government Decision Support Dashboard - FINAL PRODUCTION
// Project: Punjab GeoMethane + Machinery Analytics
// ==============================================================================

// ------------------------------------------------------------------------------
// 1. ASSET IMPORTS & ALIASING
// ------------------------------------------------------------------------------
...
// PATCH: Map the ESRI Shapefile 10-character DBF fields back to the full variable names
var fc_mapped = fc_villages.map(function(f) {
  f = f.set('Village_Name', f.get('Village_Na'));
  f = f.set('Name', f.get('Village_Na')); // Used for UI Search
  f = f.set('district', f.get('District')); // Used for UI Search
  f = f.set('Predicted_CH4_ppb', f.get('Pred_CH4'));
  f = f.set('Prime_Mover', f.get('Prime_Move'));
  f = f.set('Policy_Zone', f.get('Policy_Zon'));
  // Total_Machines is recalculated below, but we alias it anyway
  f = f.set('Total_Machines', f.get('Total_Mach')); 
  return f;
});

// For the purpose of this dashboard, we calculate dynamic indices on the fly
var mergedVillages = fc_mapped.map(function(f) {
  // Base Machinery & Schemes (Safely handle nulls)
  ...
  f = f.set('Total_Machines', totalMachines);
  ...
  var ch4 = getNum('Predicted_CH4_ppb');
  ...
  var machScore = ee.Algorithms.If(ee.Number(isMatched).eq(0), 0, // Don't score missing data as 0 machines
...
var visParams = {
  ...
  'Is_Matched': {min: 0, max: 1, palette: ['#a0aec0', '#48bb78']}, // 0 (Unmatched) = Gray, 1 (Matched) = Green
...
mergedVillages.filter(ee.Filter.gt('Total_Machines', 0)).size().evaluate(function(val) { lblMatched.setValue('Machinery Data Available (Matched): ' + val + ' / 12,467'); });
...
mergedVillages.filter(ee.Filter.eq('Top20', 1)).size().evaluate(function(val) { 
  lblTop20.setValue('Top Priority Villages: ' + (val > 0 ? val : 20)); 
});
...
function updateMapLayers() {
  ...
  // Base numeric layers
  ['Predicted_CH4_ppb', 'In_Situ', 'Ex_Situ', 'Prime_Mover', 'General', 'Scheme_Score', 'Is_Matched', 'Intervention_Priority'].forEach(function(key) {
    if (layersDict[key] && layersDict[key].getValue()) {
      ...
      if (['In_Situ', 'Ex_Situ', 'Prime_Mover', 'General', 'Scheme_Score'].indexOf(key) > -1) {
...
addLayerControl('Methane Emissions (ppb)', 'Predicted_CH4_ppb', true);
...
addLayerControl('Policy Zones', 'Policy_Zone', false);
addLayerControl('Priority Villages', 'Top_Priority', false);
...
var districtSelect = ui.Select({
  ...
  onChange: function(dist) {
    var filtered = mergedVillages.filter(ee.Filter.eq('district', dist)); // District alias
...
var villageSearch = ui.Textbox({
  ...
  onChange: function(text) {
    var filtered = mergedVillages.filter(ee.Filter.stringContains('Name', text)); // Name alias
...
Map.onClick(function(coords) {
  ...
  clickedFeature.evaluate(function(feature) {
    ...
    closeRow.add(ui.Label('Village Profile: ' + (props['Village_Name'] || 'Unknown'), {fontWeight: 'bold', fontSize: '16px', color: '#2b6cb0'}));
    ...
    // Methane & Core stats
    var ch4Val = props['Predicted_CH4_ppb'];
    ...
    inspectorPanel.add(ui.Panel({
      widgets: [
        ...
        ui.Label(props['Policy_Zone'] || 'N/A', {fontSize: '12px', margin: '0 0 0 5px'})
      ], layout: ui.Panel.Layout.flow('horizontal')
    }));
    inspectorPanel.add(ui.Panel({
      widgets: [
